...
{{--            <div class="form-group">
        ...
        <a class="btn btn-default" href="{{ route('calendar.index') }}">Back</a>
        <a class="btn btn-warning" href="{{ route('calendar.update', $calendar_event->id) }}">Edit</a>
        <form action="#/$calendar->id" method="DELETE" style="display: inline;" onsubmit="if(confirm('Delete? Are you sure?')) { return true } else {return false };"><button class="btn btn-danger" type="submit">Delete</button></form>
    </div>
</div>